...
func (s *Server) handleQuery(w http.ResponseWriter, r *http.Request, _ string) {
	...
	if err := json.Unmarshal(raw, &body); err != nil || body.Query == "" {
		httpx.WriteError(w, http.StatusBadRequest, "BadArgumentError", "Request body must contain a 'query' string.")
		return
	}

	result, err := evalKQL(s.store, body.Query)
	if err != nil {
		httpx.WriteError(w, http.StatusBadRequest, "BadArgumentError", err.Error())
		return
	}

	httpx.WriteJSON(w, http.StatusOK, queryResponse{Tables: []table{materialize(result)}})
}
