...
// timeFmt is the RFC1123 GMT layout Azure uses for message timestamps.
...
func (s *Server) mapStoreErr(w http.ResponseWriter, r *http.Request, err error) bool {
	...
	case err == nil:
		return false
	case isErr(err, queuestore.ErrQueueNotFound):
		s.writeError(w, r, azerr.QueueNotFound())
	case isErr(err, queuestore.ErrQueueExists):
		s.writeError(w, r, azerr.QueueAlreadyExists())
	case isErr(err, queuestore.ErrMessageNotFound):
		s.writeError(w, r, azerr.MessageNotFound())
	case isErr(err, queuestore.ErrPopReceipt):
